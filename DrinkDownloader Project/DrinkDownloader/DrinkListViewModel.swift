...
class DrinkListViewModel: ObservableObject {
    ...
    struct IngredientRowModel: Identifiable {
        let id: Int
        let name: String
        let measure: String
    }
    ...
    func downloadDrinks() async throws {
        ...
        print("Download \(data.count) bytes of data")
    ...
    func ingredients(for drink: Drink) -> [IngredientRowModel] {
        var result = [IngredientRowModel]()
        let names = ingredientNameList(for: drink)
        let measures = ingredientMeasureList(for: drink)
        let count = min(names.count, measures.count)
        for index in 0..<count {
            let name = names[index]
            let measure = measures[index]
            let rowModel = IngredientRowModel(id: index, name: name, measure: measure)
            result.append(rowModel)
        }
        return result
    }
    func ingredientNameList(for drink: Drink) -> [String] {
        var result = [String]()
    ...
    func ingredientMeasureList(for drink: Drink) -> [String] {
        var result = [String]()
