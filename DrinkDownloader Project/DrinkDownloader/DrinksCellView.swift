...
struct DrinksCellView: View {
    ...
    var body: some View {
        ZStack {
            VStack(spacing: 0){
                titleRow()
                glassRow()
                ForEach(viewModel.ingredients(for: drink)) { ingredient in
    ...
    private func ingredientRow(ingredient: DrinkListViewModel.IngredientRowModel) -> some View {
        HStack {
    ...
    private func cellBackground() -> some View {
        RoundedRectangle(cornerRadius: 12)
            .fill()
            .foregroundColor(.white)
            .shadow(color: .black.opacity(0.6), radius: 4.0, x: -2.0, y: 4.0)
        
    }
}

struct DrinksCellView_Previews: PreviewProvider {
    static var previews: some View {
        VStack {
            Spacer()
            DrinksCellView(viewModel: .init(), drink: Drink.preview())
            Spacer()
        }
